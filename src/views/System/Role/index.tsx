...
const STATUS_OPTIONS = [
  { label: "正常", value: RoleStatusEnum.ENABLE },
  { label: "停用", value: RoleStatusEnum.DISABLE },
];
...
const DATA_SCOPE_MAP: Record<number, string> = {
  ...
  [DataScopeEnum.DEPT_AND_CHILD]: "本部门及以下",
...
export default function Role() {
  ...
  const tableProps = useTable<true>({
    fetchData: async (pagination) => {
      ...
      const { data, err } = await getRolePage({ params });
      if (!err && data) {
        return {
          err: null,
  ...
  const handleDelete = async (id: number) => {
    const { err } = await deleteRole({ params: { id } });
    if (err) {
      message.error("删除失败");
    } else {
  ...
  const handleDeleteBatch = async () => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        const { err } = await deleteRoleList({
  ...
  const handleExport = async () => {
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        const { data, err } = await exportRole({ params });
        if (err) {
          message.error("导出失败");
        } else {
          exportFile(data!, "角色数据.xls");
  ...
  const columns: ColumnsType<RoleVO> = [
    { title: "角色编号", dataIndex: "id", width: 100, align: "center" },
    ...
    },
    {
      ...
      render: (status: number) =>
        status === RoleStatusEnum.ENABLE ? "正常" : "停用",
    },
    {
      title: "数据范围",
      dataIndex: "dataScope",
      width: 130,
      align: "center",
      render: (dataScope: number) => DATA_SCOPE_MAP[dataScope] || "-",
    },
